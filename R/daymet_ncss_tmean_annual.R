...
daymet_ncss_tmean_annual = function(path='.',
                                     year = NULL,
                                     internal = FALSE){
  ...
  # list all files
  tmin = sprintf('tmin_annavg_%s_ncss.nc', year)
  tmax = sprintf('tmax_annavg_%s_ncss.nc', year)
  ...
  if (internal == FALSE){
    raster::writeRaster(tmean_stack,
                        sprintf('tmean_annavg_%s_ncss.tif', year),
                        overwrite = TRUE)
  } else {
    return(tmean_stack)
  }

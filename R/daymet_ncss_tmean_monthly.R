...
daymet_ncss_tmean_monthly = function(path='.',
                             year = NULL,
                             internal = FALSE){
  ...
  # list all files
  tmin = sprintf('tmin_monavg_%s_ncss.nc', year)
  tmax = sprintf('tmax_monavg_%s_ncss.nc', year)
  ...
  if (internal == FALSE){
    raster::writeRaster(tmean_stack,
                        sprintf('tmean_monavg_%s_ncss.tif', year),
                        overwrite = TRUE)
  } else {
    return(tmean_stack)
  }
